...
describe('ERC20', () => {
    ...
    describe('extract @dev comments', () => {
        it('extract single line no param/return valid comments', () => {
            // verify
            expect(comments.function.get('totalSupply').dev.trim()).to.be('Total number of tokens in existence');
        });
        it('extract single line w/ param/return valid comments', () => {
            // verify
            expect(comments.function.get('balanceOf').dev.trim()).to.be('Gets the balance of the specified address.');
        });
        it('extract multiline line w/ param/return valid comments', () => {
            // verify
            expect(comments.function.get('transferFrom').dev.trim()).to.be('Transfer tokens from one address to another. '
        ...
        it('extract multiline line w/ code in valid comments', () => {
            // verify
            expect(comments.function.get('decreaseAllowance').dev.trim()).to.be('Decrease the amount of tokens that an '
        ...
        it('extract multiline line w/ link in valid comments', () => {
            // verify
            expect(comments.function.get('approve').dev.trim()).to.be('Approve the passed address to spend the '
        ...
        it('extract single line w/ _ in function name valid comments', () => {
            // verify
            expect(comments.function.get('_transfer').dev.trim()).to.be('Transfer token for a specified addresses');
        });
    });
    describe('extract @param comments', () => {
        it('extract two params w/ valid comments', () => {
            // verify
            expect(comments.function.get('allowance').param.get('owner').trim())
